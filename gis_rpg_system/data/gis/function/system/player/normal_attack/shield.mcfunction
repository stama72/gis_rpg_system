...
#damage calc
execute if score @p[tag=normal_attack] gis_job matches 1 run scoreboard players operation @s gis_damage *= #5 gis_const
execute unless score @p[tag=normal_attack] gis_job matches 1 run scoreboard players operation @s gis_damage *= #3 gis_const
#damage
execute at @s run function gis:system/damage/p_to_e_physical
say player shield attack
