...
#damage calc
execute if score @p[tag=normal_attack] gis_job matches 2 run scoreboard players operation @s gis_damage *= #3 gis_const
execute unless score @p[tag=normal_attack] gis_job matches 2 run scoreboard players operation @s gis_damage *= #1 gis_const
#damage
execute at @s run function gis:system/entity/entity_player_damage
say player fist attack
