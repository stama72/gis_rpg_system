...
#damage calc
execute if score @p[tag=normal_attack] gis_job matches 2 run scoreboard players operation @s gis_damage *= #9 gis_const
execute unless score @p[tag=normal_attack] gis_job matches 2 run scoreboard players operation @s gis_damage *= #5 gis_const
#damage
execute at @s run function gis:system/entity/entity_player_damage
say player sword attack
